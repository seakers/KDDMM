...
def extract_data_from_csv(csv_filepath, artery_problem, intpen_constr_heur, sidenum):
    ...
    for i in range(len(nfe_array)):
        ...
        if (len(current_population_truesat) > 0):
            current_population_truesat_unique = np.unique(current_population_truesat, axis = 0)
            for k2 in range(len(current_population_truesat_unique)):
                if not any(np.array_equal(f, current_population_truesat_unique[k2]) for f in fullsat): # Check if design is already in fullsat to avoid double counting
                    fullsat.append(current_population_truesat_unique[k2])
...
def plot_fraction_hypervolume_attained(nfe_hv_attained_dict, nfe_array, colour_array, line_styles_array, casename_array, savefig_name):
    fig1 = plt.figure()
    ...
    for case_key in nfe_hv_attained_dict:
        ...
        plt.plot(nfe_array, frac_runs_hv_attained, linestyle=line_styles_array[case_idx], color=colour_array[case_idx], label=casename_array[case_idx])
        case_idx += 1
...
def compute_mann_whitney_Uvals(artery_prob, hv_dict_allcases_allruns, nfe_array): # Wilcoxon Rank Sum Test
    ...
    nfe_samples_array = [0, 250, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000]
...
def plot_number_fully_satisfying_designs(num_fullsat_allcases, nfe_array, colour_array, alpha_array, line_styles_array, hatches_array, casename_array):
    fig = plt.figure()
    ...
    for i in range(num_cases):
        ...
        if all(h is None for h in hatches_array):
            plt.plot(nfe_array, num_sat_mean, linewidth=2.5, linestyle=line_styles_array[i], color=colour_array[i], label=casename_array[i])
            plt.fill_between(nfe_array, num_sat_downstd, num_sat_upstd, color=colour_array[i], linestyle=line_styles_array[i], hatch=hatches_array[i], alpha=alpha_array[i])
        else:
            plt.plot(nfe_array, num_sat_mean, linewidth=2.5, linestyle=line_styles_array[i], label=casename_array[i])
            plt.fill_between(nfe_array, num_sat_downstd, num_sat_upstd, facecolor="none", edgecolor=colour_array[i], linestyle=line_styles_array[i], hatch=hatches_array[i])
        
    plt.xlabel(r'Number of Function Evaluations',fontsize=14)
    plt.ylabel(r'Number of Fully Satisfying Designs',fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    #plt.title(plot_title)
    plt.legend(loc='upper center', bbox_to_anchor=(0.5,1.15), ncol=3, borderaxespad=0, prop={"size":12})
    plt.show()
    fig.savefig('num_fullsat.pdf', format='pdf')
...
def plot_hypervolume_stats_case_allruns(hv_dict_allruns, nfe_array, case_color, casename):
    ...
    plt.xlabel(r'Number of Function Evaluations',fontsize=14)
    plt.ylabel(r'Hypervolume',fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.title('Hypervolume (only satisfying designs) - all runs for ' + casename)
    plt.show()
    
def plot_hypervolume_stats_allcases(hv_median_dict, hv_1q_dict, hv_3q_dict, nfe_array, colour_array, alpha_array, line_styles_array, hatches_array, casename_array, plot_title, savefig_name, incl_legend):
    fig1 = plt.figure()
    ...
    for i in range(number_cases):
        ...
        if all(h is None for h in hatches_array):
            plt.plot(nfe_array, hv_median_dict['case'+str(i)], linewidth=2.5, linestyle=line_styles_array[i], color=colour_array[i], label=casename_array[i])
            plt.fill_between(nfe_array, hv_1q_dict['case'+str(i)], hv_3q_dict['case'+str(i)], color=colour_array[i], linestyle=line_styles_array[i], hatch=hatches_array[i], edgecolor="none", alpha=alpha_array[i])
        else:
            plt.plot(nfe_array, hv_median_dict['case'+str(i)], linewidth=2.5, linestyle=line_styles_array[i], label=casename_array[i])
            plt.fill_between(nfe_array, hv_1q_dict['case'+str(i)], hv_3q_dict['case'+str(i)], facecolor="none", linestyle=line_styles_array[i], hatch=hatches_array[i], edgecolor=colour_array[i])
        
        #plt.plot(nfe_array, hv_1q_dict['case'+str(i)], linestyle=line_styles_array[i], color=colour_array[i])#, label=casename_array[i]+' 1st Quartile')
        #plt.plot(nfe_array, hv_3q_dict['case'+str(i)], linestyle=line_styles_array[i], color=colour_array[i])#, label=casename_array[i]+' 3rd Quartile')
    plt.xlabel(r'Number of Function Evaluations',fontsize=14)
    plt.ylabel(r'Hypervolume',fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
...
def hypervolume_computation_all_cases(choice_model, credit_assign, case_bools_dict, prob_artery, sidenum, run_nums, marker_colours, alpha_vals, case_names):
    ...
    hv_max = get_max_hv(hv_dict_truesat_allcases)
    hv_thresh = 0.8*hv_max
        
    for i in range(num_cases):
        hv_dict_allruns = hv_dict_allcases['case'+str(i)]
        hv_dict_true_allruns = hv_dict_true_allcases['case'+str(i)]
        hv_dict_truesat_allruns = hv_dict_truesat_allcases['case'+str(i)]
...
def plotting_all_cases(num_sat_allcases, nfe_hv_attained_dict, hv_dict_med_allcases, hv_dict_1stq_allcases, hv_dict_3rdq_allcases, hv_dict_true_med_allcases, hv_dict_true_1stq_allcases, hv_dict_true_3rdq_allcases, hv_dict_truesat_med_allcases, hv_dict_truesat_1stq_allcases, hv_dict_truesat_3rdq_allcases, nfe_array0, mark_colors, alphas, linestyles_case, hatches_case, names_cases):
    print('Plotting')
    plot_number_fully_satisfying_designs(num_sat_allcases, nfe_array0, mark_colors, alphas, linestyles_case, hatches_case, names_cases)
    plot_fraction_hypervolume_attained(nfe_hv_attained_dict, nfe_array0, mark_colors, linestyles_case, names_cases, 'allcases_full')
    #plot_hypervolume_stats_allcases(hv_dict_med_allcases, hv_dict_1stq_allcases, hv_dict_3rdq_allcases, nfe_array, mark_colors, alphas, linestyles_case, hatches_case, names_cases, 'Hypervolume of Normalized Objectives', 'allcases_full', True)
    ## Plot HVs for hv_afterjump
    #plot_hypervolume_stats_allcases(hv_dict_true_med_allcases, hv_dict_true_1stq_allcases, hv_dict_true_3rdq_allcases, nfe_array, mark_colors, alphas, linestyles_case, hatches_case, names_cases, 'Hypervolume of True Objectives', 'allcases_true', True)
    plot_hypervolume_stats_allcases(hv_dict_truesat_med_allcases, hv_dict_truesat_1stq_allcases, hv_dict_truesat_3rdq_allcases, nfe_array, mark_colors, alphas, linestyles_case, hatches_case, names_cases, 'Hypervolume of True Objectives for fully satisfying designs', 'allcases_truesat', True)
    
def plot_upto_nfe(nfe_lim, hv_med_allcases, hv_1stq_allcases, hv_3rdq_allcases, nfe_array0, mark_colors, alphas, linestyles_case, hatches_case, names_cases):
    nfe_index = find_closest_index(nfe_lim, nfe_array0) # find index of closest value in nfe_array to nfe_lim
    
    nfe_array_lim = nfe_array0[:nfe_index]
    hv_med_lim_allcases = {}
    hv_1q_lim_allcases = {}
    hv_3q_lim_allcases = {}
    for i in range(len(hv_med_allcases)):
        hv_med_lim_allcases['case'+str(i)] = hv_med_allcases['case'+str(i)][:nfe_index]
        hv_1q_lim_allcases['case'+str(i)] = hv_1stq_allcases['case'+str(i)][:nfe_index]
        hv_3q_lim_allcases['case'+str(i)] = hv_3rdq_allcases['case'+str(i)][:nfe_index]
    
    plot_hypervolume_stats_allcases(hv_med_lim_allcases, hv_1q_lim_allcases, hv_3q_lim_allcases, nfe_array_lim, mark_colors, alphas, linestyles_case, hatches_case, names_cases, '', 'hv_upto_nfe', False)
...
## NOTE: For Int Pen cases, change heur_weight on line 273 to appropriate value
...
num_fullysat_cases, nfe_cdf_array, hv_dict_med_cases, hv_dict_1q_cases, hv_dict_3q_cases, hv_dict_true_med_cases, hv_dict_true_1q_cases, hv_dict_true_3q_cases, hv_dict_truesat_med_cases, hv_dict_truesat_1q_cases, hv_dict_truesat_3q_cases, Uvals_test, Uvals_test_truesat, nfe_array_1 = hypervolume_computation_all_cases(model_used, credit_assignment, cases_dict, artery_problem, sidenum, num_runs, line_colours, alpha_values, casenames)
...
casenames = ['Eps. MOEA.','All heurs','Promising heurs']
#casenames = ['Eps. MOEA','Bias Init - Orient']
plotting_all_cases(num_fullysat_cases, nfe_cdf_array, hv_dict_med_cases, hv_dict_1q_cases, hv_dict_3q_cases, hv_dict_true_med_cases, hv_dict_true_1q_cases, hv_dict_true_3q_cases, hv_dict_truesat_med_cases, hv_dict_truesat_1q_cases, hv_dict_truesat_3q_cases, nfe_array_1, line_colours, alpha_values, line_styles, case_hatches, casenames)
if artery_problem:
    plot_upto_nfe(1500, hv_dict_truesat_med_cases, hv_dict_truesat_1q_cases, hv_dict_truesat_3q_cases, nfe_array_1, line_colours, alpha_values, line_styles, case_hatches, casenames)
#print(nfe_cdf_array)
