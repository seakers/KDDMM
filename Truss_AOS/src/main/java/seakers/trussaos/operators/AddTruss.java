...
public class AddTruss implements Variation {
    ...
    public AddTruss (boolean keepFeasible, MatlabEngine eng, double[][] nodalConnArray) {
    ...
    public Solution[] evolve(Solution[] sols) {
        ...
        if (keepFeasible) {
            ...
            while (!feasibilitySameOrBetter || numberOfAttempts < 5) {
                try {
                    oldDesignTrussIntersections = findIntersectingTrusses(connectivityArray);
                ...
                if (nodesWithLowConnections.length == 0) {
                    ...
                    updatedConnectivityArray = addTrussToConnectivityArray(connectivityArray,nodesToConnect);
                ...
                try {
                    newDesignTrussIntersections = findIntersectingTrusses(updatedConnectivityArray);
                } catch (ExecutionException | InterruptedException e) {
                    e.printStackTrace();
                }
                ...
                if (oldNumberOfIntersectingTrusses >= newNumberOfIntersectingTrusses) {
            ...
            if (nodesWithLowConnections.length == 0) {
                ...
                nodesToConnect[1] = secondNodeChoices[secondNodeIndex];
                Arrays.sort(nodesToConnect);
                int[][] updatedConnectivityArray = addTrussToConnectivityArray(connectivityArray,nodesToConnect);
                newArchitecture = architecture.getArchitectureFromConnectivityArray(updatedConnectivityArray);
            }
            else {
                List<Integer> nodesList = Arrays.stream(nodesWithLowConnections).boxed().collect(Collectors.toList());
    ...
    private ArrayList<ArrayList<Integer>> findIntersectingTrusses (int[][] designConnectivityArray) throws ExecutionException, InterruptedException {
    ...
    private int[][] addTrussToConnectivityArray (int[][] oldConnectivityArray, int[] trussToAdd) {
        int addPosition = 0;
