...
[feas_struct_all_eps_fib, stab_struct_all_eps_fib] = get_feas_and_stab_scores(fibre_stiffness, epsilon_moea, feas_and_stab, num_runs, pop_size);

% Extract feasibility and stability scores for Epsilon MOEA runs using
% truss stiffness model
[feas_struct_all_eps_truss, stab_struct_all_eps_truss] = get_feas_and_stab_scores(~fibre_stiffness, epsilon_moea, feas_and_stab, num_runs, pop_size);

% Extract feasibility and stability scores for AOS MOEA runs using
% fibre stiffness model
[feas_struct_all_aos_fib, stab_struct_all_aos_fib] = get_feas_and_stab_scores(fibre_stiffness, ~epsilon_moea, feas_and_stab, num_runs, pop_size);

% Extract feasibility and stability scores for AOS MOEA runs using
% truss stiffness model
[feas_struct_all_aos_truss, stab_struct_all_aos_truss] = get_feas_and_stab_scores(~fibre_stiffness, ~epsilon_moea, feas_and_stab, num_runs, pop_size);
...
%% Functions
function [feas_struct_all, stab_struct_all] = get_feas_and_stab_scores(fib_stiff, eps_moea, const_depend, n_runs, n_pop)
