...
function dbScore = densityBiasHeuristic_2D_3x3(CA,rvar,sel,scaleFac)
    ...
    dbScore = round((exp(-(scaleFac*avgdiff))),1);
end
...
function [tCA,tAvar,trvar,bCA,bAvar,brvar] = ...
    ...
    for iy = 1:1:(sidenum-1)
        ...
        if iy == 1
            bCA = mCA;
            bAvar = Avar.*ismember(CA,mCA,'rows')';
            brvar = rvar.*ismember(CA,mCA,'rows')';
        ...
        elseif iy == 2
            tCA = mCA;
            tAvar = Avar.*ismember(CA,mCA,'rows')';
            trvar = rvar.*ismember(CA,mCA,'rows')';
