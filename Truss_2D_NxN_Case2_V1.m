% 2D NxN Square Truss Problem-- Specialized for "Case 2" Unit Cell
% "Case 2" is a 3x3 unit cell, symmetric in both x and y, with all possible
% positions for elements filled in, and a unit cell connectivity of 4 at
% the central node and the 4 edge-middle nodes
...
% FUNCTION TO GENERATE "CASE-2"-SPECIFIC ELEMENT CONNECTIVITY ARRAY
function CA = generateCA(nucFac,sidenum)
    ...
    % State connections within unit cell 
    for i = 1:1:size(cn,1)
        CA = [CA;cn(i),cn(i)-1;cn(i),cn(i)+1;cn(i),cn(i)-sidenum;...
              cn(i),cn(i)+sidenum];
        CA = [CA;cn(i)-1,cn(i)-sidenum;cn(i)-1,cn(i)+sidenum;...
              cn(i)+1,cn(i)-sidenum;cn(i)+1,cn(i)+sidenum];
    end
end
