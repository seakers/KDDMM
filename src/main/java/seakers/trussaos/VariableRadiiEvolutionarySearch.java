...
public class VariableRadiiEvolutionarySearch implements Callable<Algorithm> {
    ...
    public VariableRadiiEvolutionarySearch(Algorithm alg, TypedProperties properties, String savePath, String name, MatlabEngine eng, boolean fibreStiffness, double targetRatio, double sideNodeNumber, double[] radiusLowerBounds, double[] radiusUpperBounds, double smallestRadiusFactor) {
        this.alg = alg;
    ...
    public Algorithm call() throws IOException, ExecutionException, InterruptedException {
        ...
        VariableRadiiResultIO resultIOVariableRadii = new VariableRadiiResultIO(engine,useFibreStiffness,targetStiffnessRatio,alg.getProblem().getName(),sidenum,radiusLowerBounds,radiusUpperBounds,lowestRadiusFactor);;
        ...
        resultIOVariableRadii.savePopulationHistoryToCsvVariableRadii(allSolutions, popCsvFilename);

        String csvFilename;
        csvFilename = savePath + File.separator + alg.getClass().getSimpleName() + "_" + name + ".csv";
        resultIOVariableRadii.saveFinalResultToCsvVariableRadii(((AbstractEvolutionaryAlgorithm) alg).getPopulation(), csvFilename);
